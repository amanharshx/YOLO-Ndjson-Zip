...
impl CreateMlConverter {
    ...
    fn create_createml_segment_json(
        &self,
        images: &[&ImageEntry],
        class_names: &HashMap<i32, String>,
    ) -> String {
            ...
            .map(|img| {
                let annotations = img
                    ...
                    .map(|seg| {
                        ...
                        // Derive axis-aligned bbox from polygon vertices
                        let xs: Vec<f64> = seg.points.iter().map(|(x, _)| *x).collect();
                        let ys: Vec<f64> = seg.points.iter().map(|(_, y)| *y).collect();
                        let min_x = xs.iter().cloned().fold(f64::MAX, f64::min);
                        let max_x = xs.iter().cloned().fold(f64::MIN, f64::max);
                        let min_y = ys.iter().cloned().fold(f64::MAX, f64::min);
                        let max_y = ys.iter().cloned().fold(f64::MIN, f64::max);
    ...
    fn convert(
        ...
        for (split, images) in &splits {
            ...
            let json = match task.as_str() {
                ...
                "segment" | "semantic" => self.create_createml_segment_json(images, &class_names),
